...
angular.module('posts').controller('PostsController', [
  ...
  function ($scope, $http, $state, $stateParams, $location, Authentication, Posts, Comments, Like) {
    ...
    $scope.addComment = function (index) {
      ...
      this.newComment = '';
      comments.$save({ postId: this.post._id }, function (data) {
    ...
    $scope.likeAble = function (index) {
      ...
      likePost.$update({ postId: this.post._id }, function (data) {
...
angular.module('posts').run([
  ...
  function (Menus) {
    ...
    Menus.addSubMenuItem('topbar', 'posts', 'All Posts', 'posts');
    Menus.addSubMenuItem('topbar', 'posts', 'New Post', '/');
  }
]);'use strict';
// Setting up route
...
myApp.factory('Comments', [
  '$resource',
  function ($resource) {
    return $resource('posts/:postId/comments', { postId: '@_id' });
  }
]);
/*for likes*/
myApp.factory('Like', [
  '$resource',
  function ($resource) {
    return $resource('posts/:postId/likes', { postId: '@_id' }, { update: { method: 'PUT' } });
  }
]);'use strict';
